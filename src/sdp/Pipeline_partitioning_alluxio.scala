...
*/
  ...
  //  var broads_input1=sc.broadcast(visibility_buffer.collect())
    ...
    //  for (time <- 0 until 120) {
      ...
      sc.parallelize(initset).map(ix => timeslots_kernel(ix))
   ...
   // var broads_input2=sc.broadcast(solve.collect())
    val cor_subvis_flag: RDD[((Int, Int, Int, Int, Int, Int),Data)] = {
      ...
      sc.parallelize(initset).map(ix => cor_subvis_flag_kernel(ix))
  ...
  def cor_subvis_flag_kernel: ((Int, Int, Int, Int, Int, Int ))=> ((Int, Int, Int, Int, Int, Int), Data) = {
    case (ix) =>
     // var ix: (Int, Int, Int, Int, Int, Int) = (0, 0, 0, 0, 0, 0)
      ...
      val result = new Array[Byte](math.max(4, (scale_data * 511780275L).toInt))
      (ix, result)
  }
